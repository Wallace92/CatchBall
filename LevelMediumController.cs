...
public class LevelMediumController : MonoBehaviour
{
  ...
  //  public RectTransform textRect;  // to chyba nie jest potrzebne
    ...
    void Awake()
    {
        ...
        if (LevelChangerController.maxLevelMedium > 15 && LevelChangerController.maxLevelMedium <= levelNumbers)
        {
         //   Debug.Log("last active position " + firstTab[LevelChangerController.maxLevelMedium].GetComponent<Button>().transform.localPosition.y);
            distanceToMiddlePoint = - firstTab[LevelChangerController.maxLevelMedium-1].GetComponent<Button>().transform.localPosition.y;
        }
        else if (LevelChangerController.maxLevelMedium > levelNumbers)
        {
    ...
    void changeColors()
    {
        ...
        // POZOSTALE POZIOMY!
        if (LevelChangerController.maxLevelMedium < levelNumbers)
        {
            for (int i = 1; i < LevelChangerController.maxLevelMedium; i++)
            {
